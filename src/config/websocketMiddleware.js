...
import { WEBTRC_EXCHANGE, TRANSPORT, DISCONNECT, CONNECT, JOIN, SETUSER, GETUSER } from '../actions/types';
	...
	return store => next => action => {
		...
		switch(action.type) {
			...
			case CONNECT:
				...
				socket = io.connect('https://localhost:4443', {transports: ['websocket']});
			...
			case TRANSPORT:
				...
				break;
			case SETUSER:
				socket.emit('setUser', action.payload);
				break;
			case GETUSER:
				socket.emit('getUser', action.payload, (user) => {
					console.log(user);
